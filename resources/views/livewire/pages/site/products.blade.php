...
#products-particle-canvas { position: absolute; inset: 0; pointer-events: none; opacity: 0.4; }
...
.products-hero-content { position: relative; z-index: 10; max-width: 680px; }
.products-hero-tag {
...
.products-hero-tag span { width: 6px; height: 6px; background: var(--cyan); border-radius: 50%; display: block; }
.products-hero h1 {
  font-family: var(--font-display); font-size: clamp(2.4rem, 5vw, 3.8rem);
  font-weight: 800; color: var(--white); line-height: 1.05; letter-spacing: -0.04em; margin-bottom: 1.5rem;
}
.products-hero h1 em { color: var(--cyan); font-style: normal; }
.products-hero-sub { font-size: 1.1rem; color: rgba(255,255,255,0.6); max-width: 540px; font-weight: 300; line-height: 1.8; }

/* CATALOG SECTION */
.products-catalog { padding: 6rem; background: var(--white); position: relative; }
.catalog-header { max-width: 1200px; margin: 0 auto 4rem; }
.cat-label { font-size: 0.78rem; font-weight: 700; letter-spacing: 0.12em; color: var(--cyan); text-transform: uppercase; margin-bottom: 1rem; display: block; }
.cat-h2 { font-family: var(--font-display); font-size: clamp(2rem, 3.5vw, 2.8rem); font-weight: 800; color: var(--navy); line-height: 1.1; margin-bottom: 1rem; }
.cat-sub { font-size: 1rem; color: var(--text-secondary); max-width: 560px; font-weight: 300; }
...
.pcard.coming { cursor: default; }
.pcard-img-wrap { position: absolute; inset: 0; overflow: hidden; height: 100%; }
.pcard-img {
  position: absolute; inset: 0;
  background-size: cover; background-position: center;
  transition: transform 0.8s ease;
}
.pcard:not(.coming):hover .pcard-img { transform: scale(1.05); }
.pcard-overlay {
  position: absolute; inset: 0;
  background: linear-gradient(to bottom, rgba(13,33,55,0.18) 0%, rgba(13,33,55,0.88) 100%);
  z-index: 1;
}
.pcard-content { position: relative; z-index: 5; display: flex; flex-direction: column; height: 100%; padding: 2.5rem; color: var(--white); }
...
.pcard-body { margin-top: 2rem; }
.pcard-title { font-family: var(--font-display); font-size: 2.2rem; font-weight: 800; margin-bottom: 0.75rem; letter-spacing: -0.02em; }
.pcard-text { font-size: 1rem; color: rgba(255,255,255,0.78); line-height: 1.65; margin-bottom: 2rem; font-weight: 300; }
.pcard-btn {
...
.pcard-btn-cyan { background: var(--cyan); color: white; }
.pcard-btn-green { background: #1db954; color: white; }
.pcard-btn-disabled { background: rgba(255,255,255,0.15); color: rgba(255,255,255,0.5); pointer-events: none; border: 1px solid rgba(255,255,255,0.1); }
.pcard-btn:not(.pcard-btn-disabled):hover { gap: 1rem; box-shadow: 0 10px 20px rgba(0,0,0,0.2); }
...
.filter-tab {
  ...
  @media (max-width: 1024px) {
    .products-hero { padding: 7rem 2rem 4rem; }
    #products-radar-canvas { display: none; }
    ...
    .products-catalog { padding: 4rem 2rem; }
...
<section class="products-hero">
  ...
  <div class="products-hero-content">
    ...
    <p class="products-hero-sub">Sector-specific platforms designed with an offline-first philosophy, engineered for the infrastructure and business realities of emerging markets.</p>
  </div>
</section>
...
<section class="products-catalog">
  <div class="catalog-header">
    ...
    <p class="cat-sub">Deeply researched systems built from the ground up to solve the unique operational challenges of your industry.</p>
  </div>
  <div class="pcard-grid">
    {{-- WashOps --}}
    <a class="pcard" href="#washops" onclick="document.getElementById('washops').scrollIntoView({behavior:'smooth'}); return false;">
      <div class="pcard-img-wrap">
        <div class="pcard-img" style="background-image: url('https://lh3.googleusercontent.com/aida/ADBb0ugtz39yv0t16Pl6Vya-RxfPdyUjRqkm53GqVVxKtcF3risQzAlZ7ErADLIGl1zyDVwjL1HyjhCBHYGk7u49RDIW5nmSD0eDqMUeuaht5ZbTDrF8SGVoA_FDNraLCFr5A7Rr1sU0U4WPC7yrhMRe71lIEthpa410OkVv2EAvY_7eER0WSNV-8Ei1FxfWFqZdgvfuEwDUe-RYCWYOLhUEfZpArVG0GoJMQjisY4nGdWmBbnIlUp6wfRL2bg');"></div>
      </div>
      <div class="pcard-overlay"></div>
      <div class="pcard-content">
        <div class="pcard-top"><div class="pcard-sector">🫧 Laundry</div></div>
        <div class="pcard-body">
          <h3 class="pcard-title">WashOps</h3>
          <p class="pcard-text">Enterprise-grade desktop application with POS, real-time analytics, Kanban order board, and cloud sync — everything to run and scale a laundry business.</p>
          <span class="pcard-btn pcard-btn-cyan">Explore WashOps →</span>
        </div>
      </div>
    </a>
    {{-- ChurchOps --}}
    <a class="pcard" href="#churchops" onclick="document.getElementById('churchops').scrollIntoView({behavior:'smooth'}); return false;">
      <div class="pcard-img-wrap">
        <div class="pcard-img" style="background-image: url('https://lh3.googleusercontent.com/aida/ADBb0ujWVD-GHTi2ed6MYa1LIML9wljjzhfs5eam1YUNmftFYazmsBX7bXDlEKa04mcQZnHroewUMqJYH4rQchmQWJ3hTcT0WyvhIhXrcEAmvbOzjvQvZ6Bbm8-lN_1M3tYHbpLsC6MR8vweDJznQfhdcLbHMWNKERPsobPiVMPj5eJwDlC-KCGvUzVdoE1VHMh_r-PPIkK9bRUMdQW9hgP5x0GQnXpypkA2N5yWsZSH-tsBMLGmvm0TEtd7kHc');"></div>
      </div>
      <div class="pcard-overlay"></div>
      <div class="pcard-content">
        <div class="pcard-top"><div class="pcard-sector">⛪ Faith &amp; Community</div></div>
        <div class="pcard-body">
          <h3 class="pcard-title">ChurchOps</h3>
          <p class="pcard-text">The first offline-first church management system built for Ghanaian churches — members, finances, Mobile Money, multi-branch, and 100% uptime.</p>
    ...
    <div class="pcard coming">
      ...
      <div class="pcard-content">
        ...
        <div class="pcard-body">
          ...
          <p class="pcard-text">Hospital and pharmacy management — patient records, prescriptions, inventory, and billing. Offline-first, built for clinical environments.</p>
    ...
    <div class="pcard coming">
      ...
      <div class="pcard-content">
        ...
        <div class="pcard-body">
          ...
          <p class="pcard-text">Laboratory information management — sample tracking, test workflows, results reporting, compliance, and full audit trails.</p>
          <span class="pcard-btn pcard-btn-disabled">In Development</span>
        </div>
      </div>
    </div>
...
(function(){
  ...
  function initP(){
    ...
    for(let i=0;i<cnt;i++){
      pars.push({x:Math.random()*pc.width,y:Math.random()*pc.height,vx:(Math.random()-0.5)*0.3,vy:(Math.random()-0.5)*0.3,s:Math.random()*1.4+0.8});
    }
  }
  function animP(){
    ...
    pars.forEach((p,i)=>{
      ...
      for(let j=i+1;j<pars.length;j++){
        const q=pars[j];const dx=p.x-q.x,dy=p.y-q.y,d=Math.sqrt(dx*dx+dy*dy);
        if(d<110){pctx.strokeStyle=`rgba(0,184,219,${0.14*(1-d/110)})`;pctx.lineWidth=0.5;pctx.beginPath();pctx.moveTo(p.x,p.y);pctx.lineTo(q.x,q.y);pctx.stroke();}
      }
    });
  ...
  const RS=480; rc.width=RS; rc.height=RS;
  const CX=RS/2,CY=RS/2;
  let ang=0; const pings=[];
  const prodAngles=[45,135,225,315];
  function drawR(){
    rctx.clearRect(0,0,RS,RS);
    rctx.strokeStyle='rgba(0,184,219,0.08)'; rctx.lineWidth=1;
    for(let r=50;r<=220;r+=45){rctx.beginPath();rctx.arc(CX,CY,r,0,Math.PI*2);rctx.stroke();}
    ang+=0.015;
    const norm=(ang%(Math.PI*2));
    const g=rctx.createConicGradient(ang,CX,CY);
    g.addColorStop(0,'rgba(0,184,219,0.35)'); g.addColorStop(0.15,'rgba(0,184,219,0)');
    rctx.fillStyle=g; rctx.beginPath(); rctx.moveTo(CX,CY);
    rctx.arc(CX,CY,230,ang,ang-1); rctx.fill();
    rctx.beginPath(); rctx.moveTo(CX,CY);
    rctx.lineTo(CX+Math.cos(ang)*230,CY+Math.sin(ang)*230);
    rctx.strokeStyle='rgba(0,230,255,0.8)'; rctx.lineWidth=2; rctx.stroke();
    prodAngles.forEach(deg=>{
      const rad=deg*(Math.PI/180);
      const diff=Math.abs(norm-rad);
      if(diff<0.02) pings.push({x:CX+Math.cos(rad)*155,y:CY+Math.sin(rad)*155,r:0,a:0.8});
    });
    for(let i=pings.length-1;i>=0;i--){
      const p=pings[i]; p.r+=1.5; p.a-=0.015;
      if(p.a<=0){pings.splice(i,1);continue;}
      rctx.strokeStyle=`rgba(0,230,255,${p.a})`; rctx.lineWidth=1.2;
      for(let j=0;j<2;j++){rctx.beginPath();rctx.arc(p.x,p.y,p.r+(j*10),0,Math.PI*2);rctx.stroke();}
      rctx.fillStyle=`rgba(0,230,255,${p.a})`; rctx.beginPath();rctx.arc(p.x,p.y,4,0,Math.PI*2);rctx.fill();
    }
