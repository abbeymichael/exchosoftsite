...
<head>
    ...
    <meta name="description"
        content="{{ $metaDescription ?? 'Exchosoft Consult is a Ghana-based technology consultancy and software development company serving Black businesses across Africa, the Caribbean, and the diaspora.' }}">
...
<body class="bg-surface font-dm text-on-surface antialiased min-h-screen flex flex-col">
    ...
    <!-- Cookie Banner -->
    <div class="fixed bottom-6 left-6 right-6 md:left-auto md:right-8 md:max-w-md z-[100] transform transition-all duration-500"
        id="cookie-banner">
        <div class="border border-secondary-container/30 p-6 rounded-xl shadow-2xl backdrop-blur-xl"
                ...
                <div>
                    <p class="text-sm text-white mb-4 leading-relaxed">We use essential cookies to ensure our
                        high-fidelity platforms function as intended. View our <a class="text-secondary-fixed underline"
                            href="#">Cookie Policy</a> for more info.</p>
                    <div class="flex gap-3">
                        <button
                            class="flex-grow bg-secondary-container hover:bg-secondary-fixed-dim text-primary font-bold py-2 px-4 rounded-lg text-xs transition-colors"
                            onclick="dismissCookieBanner()">Accept All</button>
                        <button
                            class="px-4 py-2 rounded-lg text-xs font-bold text-white hover:bg-white/5 transition-colors"
                            onclick="dismissCookieBanner()">Preferences</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        (function() {
            ...
            function toggleMenu(item) {
                item.dataset.open === 'true' ? closeMenu(item, true) : openMenu(item);
            }
            ...
            document.addEventListener('click', () => document.querySelectorAll('.mega-menu-item').forEach(item =>
                closeMenu(item, true)));
        })();
