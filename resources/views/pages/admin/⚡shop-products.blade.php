...
new #[Layout('layouts.admin')] #[Title('Shop Products — ExchoSoft')] class extends Component {
    ...
    public function save(): void
    {
        ...
        $data = [
            ...
            'features' => $this->features_text ? array_filter(array_map('trim', preg_split('/[\r\n]+/', $this->features_text))) : null,
            'tech_stack' => $this->tech_stack_text ? array_filter(array_map('trim', explode(',', $this->tech_stack_text))) : null,
    ...
    {
        $this->product = ShopProduct::findOrFail($id);
        $this->product->update(['is_published' => !$this->product->is_published]);
    }
            ...
            <div class="flex flex-wrap items-center gap-2">
                <div class="relative">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search products..."
                        class="pl-9 pr-4 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-100 w-56">
                </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($this->products as $product)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-5 py-3">
                                    <span
                                        class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-700 capitalize">{{ $product->category }}</span>
                                </td>
                                <td class="px-5 py-3 text-right">
                                    <p class="font-semibold text-slate-900">GHS {{ number_format($product->price, 2) }}
                                    </p>
                                    @if ($product->sale_price)
                                        <p class="text-xs text-green-600">Sale: GHS
                                            {{ number_format($product->sale_price, 2) }}</p>
                                    @endif
                                </td>
                                <td class="px-5 py-3 text-center">
                                    <span
                                        class="font-semibold text-slate-900">{{ number_format($product->sales_count) }}</span>
                                </td>
                                <td class="px-5 py-3 text-center">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center text-sm text-slate-400">No products
                                    found. Create your first product.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="fixed inset-0 z-50 flex">
            ...
            <div class="relative ml-auto w-full max-w-xl bg-white shadow-2xl flex flex-col h-full overflow-y-auto">
                <div
                    ...
                    <h2 class="text-base font-semibold text-slate-900">{{ $editMode ? 'Edit Product' : 'New Product' }}
                    </h2>
                ...
                <form wire:submit="save" class="flex-1 px-6 py-5 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input wire:model="version" type="text" placeholder="e.g. 1.0.0"
                                class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-cyan-400">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 mb-1">Platform</label>
                            <input wire:model="platform" type="text" placeholder="Windows, Web, Cross-platform"
                                class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-cyan-400">
                        </div>
                        <div class="col-span-2">
                        ...
                        <div class="col-span-2">
                            <label class="block text-xs font-semibold text-slate-600 mb-1">
                                Linked Product Code
                                <span class="text-slate-400 font-normal ml-1">(groups on site: washops, churchops,
                                    etc.)</span>
                            </label>
                            <input wire:model="linked_product_code" type="text"
                                placeholder="e.g. washops, churchops"
                                class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-mono focus:outline-none focus:border-cyan-400">
                        </div>
                        <div class="col-span-2">
                            <label class="block text-xs font-semibold text-slate-600 mb-1">
                            ...
                            <textarea wire:model="features_text" rows="4"
                                placeholder="Revenue tracking and forecasting&#10;Order volume analytics&#10;Staff performance metrics"
                        ...
                        <div class="col-span-2">
                            ...
                            <input wire:model="tech_stack_text" type="text"
                                placeholder="C#, .NET, SQLite, WPF, AWS"
                                class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:border-cyan-400">
                        </div>
                        <div class="col-span-2">
                        ...
                        <div class="col-span-2 flex flex-wrap gap-4 pt-1">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input wire:model="requires_license" type="checkbox"
                                    class="rounded border-slate-300 text-cyan-600">
