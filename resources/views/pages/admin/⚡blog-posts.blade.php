...
new #[Layout('layouts.admin')] #[Title('Blog Posts — ExchoSoft')] class extends Component {
    ...
    {
        return BlogPost::with('author')->when($this->search, fn($q) => $q->where('title', 'like', '%' . $this->search . '%'))->when($this->filterStatus, fn($q) => $q->where('status', $this->filterStatus))->when($this->filterCategory, fn($q) => $q->where('category', $this->filterCategory))->latest()->paginate(15);
    }
}; ?>
    ...
    <div class="space-y-5">
        ...
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($this->posts as $post)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-5 py-3">
                                    <span
                                        class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-700 capitalize">{{ $post->category }}</span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center text-sm text-slate-400">No blog posts
                                    yet. Write your first post!</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if ($this->posts->hasPages())
                <div class="border-t border-slate-100 px-5 py-4">{{ $this->posts->links() }}</div>
            @endif
        </div>
    </div>
        ...
        <div class="fixed inset-0 z-50 flex">
            ...
            <div class="relative ml-auto w-full max-w-4xl bg-white shadow-2xl flex flex-col h-full overflow-y-auto">
                <div
                    ...
                    <h2 class="text-base font-semibold text-slate-900">{{ $editMode ? 'Edit Post' : 'New Blog Post' }}
                    </h2>
