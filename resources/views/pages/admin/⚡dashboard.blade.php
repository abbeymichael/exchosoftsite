...
new #[Layout('layouts.admin')] #[Title('Dashboard — ExchoLicense')] class extends Component {
    ...
    public function apiStats(): array
    {
        ...
        $success = ApiRequestLog::where('created_at', '>=', now()->subHours($window))
            ->where('success', true)
            ->count();
    ...
    public function validationStats(): array
    {
        ...
        return [
            ...
            // Top failure reasons
            'failure_reasons' => (clone $base)->where('success', false)->whereNotNull('failure_reason')->select('failure_reason', \Illuminate\Support\Facades\DB::raw('COUNT(*) as count'))->groupBy('failure_reason')->orderByDesc('count')->limit(5)->get()->toArray(),
        ];
    }
    ...
    {
        return ValidationLog::with('license.product')->latest('created_at')->limit(8)->get();
    }
...
<div>
    ...
    <div class="space-y-6">
        ...
        {{-- Hero banner --}}
        <div
            class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-cyan-600 to-blue-600 p-6 text-white shadow-lg">
            <div class="relative z-10">
                ...
                <div class="mt-3 flex flex-wrap gap-3">
                    <span
                        class="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-xs font-medium">
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
                ...
                <p class="mt-4 text-2xl font-bold text-slate-900">{{ number_format($this->stats['total_licenses']) }}
                </p>
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
                ...
                <p class="mt-4 text-2xl font-bold text-slate-900">{{ number_format($this->stats['total_customers']) }}
                </p>
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
                ...
                <p class="mt-4 text-2xl font-bold text-slate-900">{{ number_format($this->stats['total_activations']) }}
                </p>
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
                ...
                <p class="mt-4 text-2xl font-bold text-slate-900">{{ number_format($this->stats['expiring_soon']) }}
                </p>
                <p class="mt-0.5 text-sm text-slate-500">Expiring (30d)</p>
                <p class="mt-2 text-xs font-medium text-amber-600">GHS
                    {{ number_format($this->stats['monthly_revenue'], 2) }} MRR</p>
            </div>
        ...
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <div>
                    ...
                    <p class="mt-0.5 text-xs text-slate-400">Request counts and success rates in the last
                        {{ $statsWindow }} hours</p>
                </div>
            ...
            <div class="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100">
                ...
                <div class="px-6 py-4 text-center">
                    <p
                        class="text-2xl font-bold {{ $this->apiStats['failed'] > 0 ? 'text-red-600' : 'text-slate-400' }}">
                    ...
                    <thead>
                        <tr class="bg-slate-50/70 border-b border-slate-100">
                            ...
                            <th
                                ...
                                Success Rate</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 w-40">
                                Rate Bar</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse($this->endpointStats as $ep)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-3">
                                    <span
                                        class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-mono font-semibold
                                ...
                                <td class="px-6 py-3 text-right font-semibold text-slate-900">
                                    {{ number_format($ep['total']) }}</td>
                                <td class="px-6 py-3 text-right text-green-600 font-medium">
                                    {{ number_format($ep['success']) }}</td>
                                <td
                                    class="px-6 py-3 text-right {{ $ep['failed'] > 0 ? 'text-red-600 font-medium' : 'text-slate-400' }}">
                                    {{ number_format($ep['failed']) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-sm text-slate-400">No API
                                    requests in the selected window.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
                ...
                <div class="px-6 py-5 space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <span
                                class="text-sm font-semibold text-slate-900">{{ number_format($vs['validate_total']) }}</span>
                            @if ($vs['validate_total'] > 0)
                                <span class="text-xs text-green-600">{{ number_format($vs['validate_success']) }}
                                    ✓</span>
                                <span class="text-xs text-red-500">{{ number_format($vs['validate_failed']) }}
                                    ✗</span>
                            @endif
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="text-sm font-semibold text-slate-900">{{ number_format($vs['status_total']) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-slate-600">Deactivations</span>
                        <span
                            class="text-sm font-semibold text-slate-900">{{ number_format($vs['deactivate_total']) }}</span>
                    </div>

                    @if (count($vs['failure_reasons']) > 0)
                        <div class="pt-3 border-t border-slate-100">
                            <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Top Failure
                                Reasons</p>
                            @foreach ($vs['failure_reasons'] as $reason)
                                <div class="flex items-center justify-between py-1">
                                    <span
                                        class="text-xs font-mono text-slate-600">{{ $reason['failure_reason'] }}</span>
                                    <span
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
                ...
                <div class="px-6 py-5 space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span
                                class="h-2 w-2 rounded-full {{ $health['failed_last_hour'] > 10 ? 'bg-red-500' : 'bg-green-500' }}"></span>
                            <span class="text-sm text-slate-600">Failed validations (1h)</span>
                        </div>
                        <span
                            class="text-xs font-medium {{ $health['failed_last_hour'] > 10 ? 'text-red-600' : 'text-green-600' }}">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span
                                class="h-2 w-2 rounded-full {{ $health['suspicious_activations'] > 0 ? 'bg-amber-500' : 'bg-green-500' }}"></span>
                            <span class="text-sm text-slate-600">Suspicious activations (24h)</span>
                        </div>
                        <span
                            class="text-xs font-medium {{ $health['suspicious_activations'] > 0 ? 'text-amber-600' : 'text-green-600' }}">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="text-xs font-medium {{ $this->apiStats['success_rate'] >= 95 ? 'text-green-600' : ($this->apiStats['success_rate'] >= 80 ? 'text-amber-600' : 'text-red-600') }}">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    ...
                    <a href="{{ route('admin.licenses') }}" wire:navigate
                        class="text-xs font-medium text-cyan-600 hover:text-cyan-700">View all →</a>
                </div>
                <div class="divide-y divide-slate-100">
                    @forelse($this->recentLicenses as $license)
                        <div class="flex items-center justify-between px-6 py-3">
                            <div>
                                <p class="text-sm font-mono font-medium text-slate-900">{{ $license->license_key }}
                                </p>
                                <p class="text-xs text-slate-500">{{ $license->customer?->name ?? '—' }} ·
                                    {{ $license->product?->name ?? '—' }}</p>
                            </div>
                            <span
            ...
            <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    ...
                    <a href="{{ route('admin.audit-logs') }}" wire:navigate
                        class="text-xs font-medium text-cyan-600 hover:text-cyan-700">View logs →</a>
                </div>
                <div class="divide-y divide-slate-100">
                        ...
                        <div class="flex items-center justify-between px-6 py-3">
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <p class="text-xs font-mono text-slate-600 truncate">
                                        {{ $log->license_key ?? '—' }}</p>
                                </div>
                            ...
                            <p class="ml-3 text-xs text-slate-400 flex-shrink-0">
        ...
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 overflow-hidden">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                ...
                <a href="{{ route('admin.activations') }}" wire:navigate
                    class="text-xs font-medium text-cyan-600 hover:text-cyan-700">View all →</a>
            </div>
            <div class="divide-y divide-slate-100">
                    ...
                    <div class="flex items-center justify-between px-6 py-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-shrink-0">
                                ...
                                @if ($appType === 'web')
                                    <span
                                        class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-blue-50 text-blue-600 text-xs font-bold">W</span>
                                @elseif($appType === 'cloud')
                                    <span
                                        class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-purple-50 text-purple-600 text-xs font-bold">C</span>
                                @elseif($appType === 'hybrid')
                                    <span
                                        class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-amber-50 text-amber-600 text-xs font-bold">H</span>
                                @else
                                    <span
                                        class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 text-slate-500 text-xs font-bold">D</span>
                                @endif
                            </div>
                            <div>
                                <p class="text-sm font-medium text-slate-900">
                                    {{ $activation->device_name ?? $activation->device_id }}</p>
                                <p class="text-xs text-slate-500">{{ $activation->license?->customer?->name ?? '—' }}
                                    · {{ $activation->ip_address }} · {{ ucfirst($appType) }}</p>
                            </div>
                        </div>
                        <span
                            class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium
            ...
            <div class="rounded-2xl bg-amber-50 border border-amber-200 overflow-hidden">
                ...
                <div class="divide-y divide-amber-100">
                    @foreach ($this->expiringLicenses as $license)
                        <div class="flex items-center justify-between px-6 py-3">
                            <div>
                                <p class="text-sm font-mono font-medium text-amber-900">{{ $license->license_key }}
                                </p>
                                <p class="text-xs text-amber-700">{{ $license->customer?->name ?? '—' }} ·
                                    {{ $license->product?->name ?? '—' }}</p>
                            </div>
                            <p class="text-sm font-semibold text-amber-800">
                                {{ $license->expires_at->diffForHumans() }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
