...
new #[Layout('layouts.admin')] #[Title('Edit Page — ExchoSoft')] class extends Component {
    ...
    public function mount(string $key): void
    {
        ...
        $this->fill([
            ...
            'schema_markup_raw' => $this->page->schema_markup ? json_encode($this->page->schema_markup, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '',
            'extra_raw' => $this->page->extra ? json_encode($this->page->extra, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '',
    ...
    public function save(): void
    {
        ...
        $this->page->snapshot($changedBy, filled($this->saveNote) ? $this->saveNote : 'Admin edit via Page Editor');
...
<div>
    ...
    @if (session('success'))
        <div
            class="mb-5 rounded-xl bg-green-50 border border-green-100 px-4 py-3 text-sm text-green-700 flex items-center gap-2">
    ...
    <div class="space-y-5">
        ...
        {{-- Header bar --}}
        <div
            class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl bg-white border border-slate-100 shadow-sm px-5 py-4">
            <div class="flex items-center gap-3">
                <span
                    class="font-mono text-xs bg-slate-100 text-slate-600 rounded-lg px-2.5 py-1.5">{{ $page->key }}</span>
                @if ($this->liveUrl)
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.pages.versions', $page->key) }}"
                    ...
                    Version History
                    <span
                        class="rounded-full bg-slate-100 px-1.5 text-[10px] font-semibold text-slate-500">{{ $page->versions()->count() }}</span>
                </a>
                <label class="inline-flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" wire:model="is_active" class="sr-only peer">
                    <div
                        class="relative w-9 h-5 bg-slate-200 peer-checked:bg-green-500 rounded-full transition-colors after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-4">
                    </div>
        ...
        <div class="flex gap-1 rounded-xl bg-slate-100 p-1 w-fit">
            @foreach ([['banner', 'Banner', '<path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>'], ['seo', 'SEO', '<path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>'], ['og', 'Open Graph', '<path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/>'], ['twitter', 'Twitter', '<path stroke-linecap="round" stroke-linejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"/>'], ['advanced', 'Advanced', '<path stroke-linecap="round" stroke-linejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/>']] as [$tab, $label, $icon])
                <button wire:click="setTab('{{ $tab }}')"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all
                           {{ $activeTab === $tab ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2">{!! $icon !!}</svg>
                    {{ $label }}
                </button>
            @endforeach
        </div>
        ...
        <div class="rounded-2xl bg-white border border-slate-100 shadow-sm p-6">
            ...
            @if ($activeTab === 'banner')
                <div class="space-y-5">
                    <div>
                        <h3 class="text-sm font-semibold text-slate-900 mb-1">Page Title <span
                                class="text-red-400">*</span></h3>
                        <p class="text-xs text-slate-400 mb-2">The HTML <code>&lt;title&gt;</code> tag — shown in
                            browser tab and used as a fallback for SEO/OG.</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input wire:model="banner_heading" type="text"
                                placeholder="e.g. Built From Here. Built For Here."
                    ...
                    <div>
                        ...
                        <input wire:model="banner_image" type="text"
                            placeholder="/assets/images/about-banner.jpg or https://…"
                            class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:outline-none focus:border-cyan-400">
                        <p class="mt-1 text-xs text-slate-400">Background or hero image for the page banner. Leave blank
                            to use the default canvas/gradient.</p>
                        @if ($banner_image)
                            <div class="mt-2 rounded-lg overflow-hidden border border-slate-200 h-28">
                                <img src="{{ $banner_image }}" alt="Banner preview"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input wire:model="banner_cta_url" type="text"
                    ...
                    @if ($banner_heading || $banner_subheading)
                        <div class="rounded-xl border border-slate-200 bg-slate-900 p-6 mt-2">
                            <p class="text-xs text-slate-500 mb-3 uppercase font-semibold tracking-wide">Banner Preview
                            </p>
                            @if ($banner_heading)
                            ...
                            @if ($banner_cta_text)
                                <div class="mt-3">
                                    <span
                                        class="inline-block rounded-lg bg-cyan-500 text-white text-xs font-semibold px-3 py-1.5">{{ $banner_cta_text }}</span>
                                </div>
                            @endif
                        </div>
                    @endif
            ...
            @if ($activeTab === 'seo')
                <div class="space-y-5">
                    <div class="rounded-xl bg-blue-50 border border-blue-100 px-4 py-3 text-xs text-blue-700">
                        <strong>SEO tip:</strong> Meta title should be 50–60 characters. Meta description should be
                        120–160 characters.
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center justify-between">
                            ...
                            <span
                                class="text-xs {{ strlen($meta_title) > 60 ? 'text-red-500 font-semibold' : (strlen($meta_title) >= 50 ? 'text-green-600' : 'text-slate-400') }}">
                        ...
                        @if ($meta_title)
                            <div class="mt-2 rounded-lg border border-slate-200 bg-white p-3">
                                <p class="text-[10px] text-slate-400 uppercase font-semibold mb-1">Google SERP Preview
                                </p>
                                <div class="text-xs text-green-700">{{ url($canonical_url ?: '/') }}</div>
                                <div class="text-sm font-medium text-blue-700 mt-0.5 truncate">{{ $meta_title }}
                                </div>
                                @if ($meta_description)
                                    <div class="text-xs text-slate-500 mt-0.5 line-clamp-2">{{ $meta_description }}
                                    </div>
                                @endif
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center justify-between">
                            ...
                            <span
                                class="text-xs {{ strlen($meta_description) > 160 ? 'text-red-500 font-semibold' : (strlen($meta_description) >= 120 ? 'text-green-600' : 'text-slate-400') }}">
                    ...
                    <div>
                        ...
                        <input wire:model="meta_keywords" type="text" placeholder="comma, separated, keywords"
                            class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:outline-none focus:border-cyan-400">
                        <p class="mt-1 text-xs text-slate-400">Low SEO impact in modern search, but useful for indexers
                            and internal search.</p>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Canonical URL</label>
                        <input wire:model="canonical_url" type="text"
                            placeholder="/about or https://exchosoft.com/about"
                            class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:outline-none focus:border-cyan-400">
                        <p class="mt-1 text-xs text-slate-400">Tells search engines the preferred URL for this page.
                            Usually the page's own path.</p>
                    </div>
                </div>
            @endif
                ...
                <div class="space-y-5">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-slate-400 mt-0.5">Controls how this page looks when shared on
                                Facebook, LinkedIn, Slack, etc.</p>
                        </div>
                    ...
                    <div>
                        ...
                        <input wire:model="og_image" type="text"
                            placeholder="https://… or /assets/images/og-about.jpg"
                            class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:outline-none focus:border-cyan-400">
                        <p class="mt-1 text-xs text-slate-400">Recommended: 1200×630px. Shown as the link preview image
                            on social platforms.</p>
                        @if ($og_image)
                            <div class="mt-2 rounded-lg overflow-hidden border border-slate-200"
                                style="max-width:400px;height:120px;">
                                <img src="{{ $og_image }}" alt="OG image preview"
                                    class="w-full h-full object-cover">
                            </div>
                        @endif
                    </div>
                    ...
                    @if ($og_title || $og_description)
                        <div class="rounded-xl border border-slate-200 overflow-hidden max-w-sm">
                            ...
                            @if ($og_image)
                                <div class="h-32 bg-slate-100"><img src="{{ $og_image }}" alt=""
                                        class="w-full h-full object-cover"></div>
                            @else
                                <div
                                    class="h-32 bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center">
                ...
                <div class="space-y-5">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-slate-400 mt-0.5">Controls how this page looks when shared on
                                Twitter/X. Falls back to OG values if blank.</p>
                        </div>
                    ...
                    <div>
                        ...
                        <select wire:model="twitter_card"
                            class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:outline-none focus:border-cyan-400">
                            <option value="summary_large_image">summary_large_image (recommended — large banner)
                            </option>
                    ...
                    <div>
                        ...
                        <input wire:model="twitter_title" type="text"
                            placeholder="Overrides OG title on Twitter/X…"
                            class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:outline-none focus:border-cyan-400">
                    </div>
                    ...
                    <div>
                        ...
                        <input wire:model="twitter_image" type="text"
                            placeholder="https://… or /assets/images/twitter-about.jpg"
                            class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:outline-none focus:border-cyan-400">
                        <p class="mt-1 text-xs text-slate-400">Recommended: 1200×600px for summary_large_image.
                            Fallback: OG image.</p>
                        @if ($twitter_image)
                            <div class="mt-2 rounded-lg overflow-hidden border border-slate-200"
                                style="max-width:400px;height:120px;">
                                <img src="{{ $twitter_image }}" alt="Twitter image preview"
                                    class="w-full h-full object-cover">
                            </div>
                        @endif
                    </div>
            ...
            @if ($activeTab === 'advanced')
                <div class="space-y-6">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Schema Markup
                            (JSON-LD)</label>
                        <p class="text-xs text-slate-400 mb-2">
                            Structured data injected as a <code class="bg-slate-100 px-1 rounded">&lt;script
                                type="application/ld+json"&gt;</code> tag.
                            Must be valid JSON. <a href="https://schema.org" target="_blank"
                                class="text-cyan-600 hover:underline">schema.org reference ↗</a>
                        </p>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mb-2">
                            Free-form JSON for page-specific overrides, feature flags, or CMS keys. Available in the
                            Livewire component as <code class="bg-slate-100 px-1 rounded">$page->extra['key']</code>.
                        </p>
        ...
        <div class="rounded-2xl bg-white border border-slate-100 shadow-sm px-5 py-4">
            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                <div class="flex-1">
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">Save Note <span
                            class="text-slate-400 font-normal">(optional — stored in version history)</span></label>
